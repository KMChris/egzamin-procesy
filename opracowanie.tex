...
\begin{odpowiedź}
    ...
    \mathbb{P}(X_{n+1}=x_{x+1}\mid X_n=x_n,\ldots ,X_0=x_0)\\
    = \mathbb{P}(X_{n+1}=x_{n+1}\mid X_n=x_n),
    \]
    Gdzie $n \in N$ i $x_0, x_1, \ldots, x_{n+1} \in S$.\\
...
\begin{odpowiedź}
    ...
    Własności:
    \begin{enumerate}
        \item $p(x, y) > 0 \forall (x, y) \in S$
        \item $\sum_{y} p(x, y) = 1 \forall x \in S$
    \end{enumerate}
\end{odpowiedź}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{pytanie}
\begin{odpowiedź}
    Rozkład prawdopodobieństwa $\mi = (\mi (x) : x \in s)$
